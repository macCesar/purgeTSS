...
## Breaking changes
**This is the last breaking change for the next 6 months... We promise!! ;-).**

Within this time frame, we will be releasing new features and bug fixes only.
...
Start by downloading them from their official websites:
...
**1. It will generate a custom `./purgetss/fonts.tss` file with all the unicode characters and style rules:**
...
**PurgeTSS** will use the font's file name as the style rule name.

**You can change it by renaming the fonts file name**.
...
**2. It will copy the font files to `./app/assets/fonts` folder and renaming them to their corresponding PostScript name to work on both iOS and Android Apps:**
```bash
# ./app/assets/fonts
app
└─ assets
...
**3. And when using the `--modules` option, it will generate a `./app/lib/purgetss.fonts.js` CommonJS module file:**
...
**Remember that you use any Serif, Sans-Serif, Cursive, Fantasy or Monospace font families to your custom `fonts` file, by copying the TrueType and OpenType fonts into the `./purgetss/fonts` folder.**
