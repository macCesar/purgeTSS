...
- **Reduced purging time 56% ( on average )**
- iOS Only
...
- You can set: `delay`, `duration`, `rotation`, `scaling`, `repetition`, `animation curve`, you can animate properties like background color, opacity, width, height and more.
...
### Usage
To use it, create an Alloy element that will hold the transformation values. And set its 'module' property to `module='purgetss.ui'`

You can set any of the sizing, position and color properties available in `tailwind.tss`.
...
### Example 2
You can create a more complex animations, combining properties to different `states` like when `opening` or `closing` an animation.

```xml
<Alloy>
  <Window class="keep-screen-on">
    <View class="vertical">
      <Button class="w-48 mt-1 bg-purple-500 rounded ios:mt-16 text-purple-50" android:onClick="do3Squares" ios:onSingletap="do3Squares" title="3 Squares"/>
      <Button platform="ios" class="w-48 mt-1 mt-2 bg-purple-500 rounded text-purple-50" android:onClick="doChangeColors" ios:onSingletap="doChangeColors" title="Toggle Colors"/>
      <Button class="w-48 mt-1 mt-2 bg-purple-500 rounded text-purple-50" android:onClick="doRetweet" ios:onSingletap="doRetweet" title="Toggle Re-Tweet"/>
    ...
    <View id="square3" class="mt-8 bg-blue-500 w-14 h-14 rounded-xl" android:onClick="doTransparency" ios:onSingletap="doTransparency" />

    <View id="retweetView" class="w-screen h-48 bg-gray-800 vertical -mb-52 rounded-2xl" android:onClick="doRetweet" ios:onSingletap="doRetweet">
    ...
    <Animation id="squaresWidth" module="purgetss.ui" class="ease-out delay-50 open:w-11/12 close:w-28 debug-on" />
    <Animation id="transparency" module="purgetss.ui" class="open:duration-150 close:duration-300 open:mt-(null) close:mt-8 open:w-10/12 open:h-11/12 close:w-14 close:h-14 open:opacity-50 close:opacity-100" />
    <Animation id="retweet" module="purgetss.ui" class="open:duration-200 close:duration-150 open:-mb-16 close:-mb-52" />
    <Animation id="changeColor" module="purgetss.ui" platform="ios" class="delay-25 open:bg-purple-500 close:bg-blue-700 debug-on" />
  </Window>
</Alloy>
```
...
### Example 3
The following example `applies` the properties to prevent the animations from showing.
...
Use it in combination with `from-{color}` and `to-{color}` properties.
