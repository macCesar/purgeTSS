		...
		if (typeof value === 'string' && value !== 'currentColor') {
			...
			_.each(value, (shadeValue, shadeName) => {
				if (shadeName === 'global' || shadeName === 'default' || shadeName === 'DEFAULT') {
					convertedStyles += singleValue(`${rule}`, `${checkModifier(modifier)}`, `${property}`, `${parseValue(shadeValue)}`);
				} else {
					if (modifier === 'ios' || modifier === 'android' || modifier.includes('[')) {
						convertedStyles += singleValue(`${rule}`, `${shadeName}${checkModifier(modifier)}`, `${property}`, `${parseValue(shadeValue)}`);
					} else {
						if (modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT') {
							convertedStyles += singleValue(`${rule}`, `${shadeName}`, `${property}`, `${parseValue(shadeValue)}`);
						} else {
							convertedStyles += singleValue(`${rule}`, `${modifier}-${shadeName}`, `${property}`, `${parseValue(shadeValue)}`);
						}
					}
				}
			});
...
function gradientColorStops(tailwindColors) {
		...
		_.each(tailwindColors, (value, modifier) => {
			if (typeof value === 'string') {
				if (value === 'transparent') {
					...
					convertedStyles += `'.${rule}${modifier}': { backgroundGradient: { colors: [ '` + addTransparency(value) + `', '${value}' ] } }\n`;
				}
			} else {
				_.each(value, (shadeValue, shadeName) => {
					if (shadeName === 'global' || shadeName === 'default' || shadeName === 'DEFAULT') {
						convertedStyles += `'.${rule}${modifier}': { backgroundGradient: { colors: [ '` + addTransparency(shadeValue) + `', ${parseValue(shadeValue)} ] } }\n`;
					} else {
						if (modifier === 'ios' || modifier === 'android' || modifier.includes('[') || modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT') {
							convertedStyles += `'.${rule}${shadeName}${checkModifier(modifier)}': { backgroundGradient: { colors: [ '` + addTransparency(shadeValue) + `', ${parseValue(shadeValue)} ] } }\n`;
						} else {
							convertedStyles += `'.${rule}${modifier}-${shadeName}': { backgroundGradient: { colors: [ '` + addTransparency(shadeValue) + `', ${parseValue(shadeValue)} ] } }\n`;
						}
					}
				});
			}
	...
	_.each(rulesAndProperties, rule => {
		...
		_.each(tailwindColors, (value, modifier) => {
			if (typeof value === 'string' && value !== 'currentColor') {
				convertedStyles += `'.${rule}${modifier}': { backgroundGradient: { colors: [ '${value}' ] } }\n`;
			} else if (value !== 'currentColor') {
				_.each(value, (shadeValue, shadeName) => {
					if (shadeName === 'global' || shadeName === 'default' || shadeName === 'DEFAULT') {
						convertedStyles += `'.${rule}${modifier}': { backgroundGradient: { colors: [ ${parseValue(shadeValue)} ] } }\n`;
					} else {
						if (modifier === 'ios' || modifier === 'android' || modifier.includes('[') || modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT') {
							convertedStyles += `'.${rule}${shadeName}${checkModifier(modifier)}': { backgroundGradient: { colors: [ ${parseValue(shadeValue)} ] } }\n`;
						} else {
							convertedStyles += `'.${rule}${modifier}-${shadeName}': { backgroundGradient: { colors: [ ${parseValue(shadeValue)} ] } }\n`;
						}
					}
				});
			}
...
function opacity(tailwindOpacity) {
	...
	_.each(tailwindOpacity, (value, modifier) => {
		if (typeof value === 'object') {
			_.each(value, (elValue, laPropiedad) => {
				convertedStyles += singleValue(`.opacity-${laPropiedad}`, checkModifier(modifier), 'opacity', parseFloat(elValue));
			});
		} else {
...
function addTransparency(hexValue) {
	try {
		let value = hexValue;
		switch (hexValue.length) {
			case 4:
				value = `#0${hexValue[ 1 ]}${hexValue[ 2 ]}${hexValue[ 3 ]}`
				break;
			case 7:
				value = `#00${hexValue[ 1 ]}${hexValue[ 2 ]}${hexValue[ 3 ]}${hexValue[ 4 ]}${hexValue[ 5 ]}${hexValue[ 6 ]}`
				break;
			case 9:
				value = `#00${hexValue[ 3 ]}${hexValue[ 4 ]}${hexValue[ 5 ]}${hexValue[ 6 ]}${hexValue[ 7 ]}${hexValue[ 8 ]}`
				break;
		}

		return value;
	} catch (error) {
		console.log('Error:', hexValue, position);
	}
