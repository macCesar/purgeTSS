...
function backgroundGradient(modifiersAndValues) {
		...
		_.each(modifiersAndValues, (value, modifier) => {
			if (typeof value === 'object') {
				_.each(value, (_value, _modifier) => {
					convertedStyles += `'.${rule}-${setModifier2(modifier, rule)}${setModifier2(_modifier)}': ` + _.replace(_.replace(properties, new RegExp("{transparentValue}", "g"), `${addTransparencyToValue(parseValue(_value))}`), new RegExp("{value}", "g"), parseValue(_value)) + '\n';
				});
			} else {
	...
	_.each(objectPosition, (properties, rule) => {
		_.each(modifiersAndValues, (value, modifier) => {
			if (typeof value === 'object') {
				_.each(value, (_value, _modifier) => {
					convertedStyles += `'.${rule}-${setModifier2(modifier, rule)}${setModifier2(_modifier)}': ` + _.replace(properties, new RegExp("{value}", "g"), parseValue(_value)) + '\n';
				});
			} else {
...
function checkColorClasses(cleanClassName) {
	return (
		...
		cleanClassName.startsWith('bg-focused-') ||
		...
		cleanClassName.startsWith('selected-subtitle-') ||
		...
		cleanClassName.startsWith('title-text-') ||
