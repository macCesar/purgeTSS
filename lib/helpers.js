		...
		_.each(modifiersAndValues, (value, modifier) => {
			if (typeof value === 'object') {
				_.each(value, (_value, _modifier) => {
					convertedStyles += `'.${rule}-${modifier}${setModifier(_modifier)}': ` + _.replace(properties, new RegExp("{value}", "g"), parseValue(_value, minusSigns)) + '\n';
				});
			} else {
	...
	_.each(_applyClasses, (value, modifier) => {
		...
		if (indexOfModifier !== -1) {
			...
			_.each([...value], searchClass => {
				...
				if (indexOfClass !== -1) {
					compoundClasses.push(twStylesArray[indexOfClass].replace(`'.${searchClass}': `, '').replace(/{(.*)}/, "$1").trim());
				}
			});
