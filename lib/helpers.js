		...
		if (indexOfModifier !== -1) {
			...
			_.each([...values], searchClass => {
				...
				if (searchClass.includes('(')) {
					...
					let transparency = Math.round(decimalValue * 255 / 100).toString(16).padStart(2, '0');
			...
			if (classesWithOpacityValues.length) {
				classesWithOpacityValues.forEach(opacityValue => {
					let opacityIndex = findIndexOfClassName(`'.${opacityValue.classNameWithTransparency}`, twClassesArray);
					if (opacityIndex > -1) {
						let defaultHexValue = (twClassesArray[opacityIndex].includes('from')) ? twClassesArray[opacityIndex].match(/\#[0-9a-f]{6}/g)[1] : twClassesArray[opacityIndex].match(/\#[0-9a-f]{6}/i)[0];
						let classWithoutDecimalOpacity = `${twClassesArray[opacityIndex].replace(new RegExp(defaultHexValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), `#${opacityValue.transparency}${defaultHexValue.substring(1)}`)}`;
