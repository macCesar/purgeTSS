...
function processColorProperties(header, rule, property, data) {
	...
	_.each(data, (value, modifier) => {
		if (typeof value === 'string' && value !== 'currentColor') {
			...
			_.each(value, (shadeValue, shadeModifier) => {
				if (defaultModifier(shadeModifier)) {
					convertedStyles += singleValue(rule, modifier, property, `'${toHex(shadeValue)}'`);
				} else if (deviceModifier(shadeModifier)) {
					convertedStyles += singleValue(`${rule}${setModifier(modifier)}`, shadeModifier, property, `'${toHex(shadeValue)}'`);
				} else {
					if (deviceModifier(modifier)) {
						convertedStyles += singleValue(rule, `${setModifier(shadeModifier)}${setModifier(modifier)}`, property, `'${toHex(shadeValue)}'`);
					} else {
						if (defaultModifier(modifier)) {
							convertedStyles += singleValue(rule, shadeModifier, property, `'${toHex(shadeValue)}'`);
						} else {
							convertedStyles += singleValue(rule, `${setModifier(modifier)}${setModifier(shadeModifier)}`, property, `'${toHex(shadeValue)}'`);
						}
					}
				}
			});
...
function backgroundSelectedColor(tailwindColors) {
	return processColorProperties('backgroundSelectedColor', '.bg-selected', 'backgroundSelectedColor', tailwindColors);
}
...
function touchFeedbackColor(tailwindColors) {
	return processColorProperties('touchFeedbackColor', '.feedback', 'touchFeedback: true, touchFeedbackColor', tailwindColors);
}
...
function fontFamily(values) {
	...
	_.each(values, (value, modifier) => {
		if (defaultOrDeviceModifier(modifier)) {
			if (typeof value === 'object') {
				_.each(value, (theValue, theModifier) => {
					convertedStyles += multiValues(`.font${setModifier(theModifier)}`, modifier, 'font', ` fontFamily: '${theValue}'`);
				});
			} else {
			...
			if (typeof value === 'object') {
				_.each(value, (theValue, theModifier) => {
					convertedStyles += multiValues(`.font${setModifier(modifier)}`, theModifier, 'font', ` fontFamily: '${theValue}'`);
				});
			} else {
				convertedStyles += multiValues('.font', modifier, 'font', ` fontFamily: '${value}'`);
			}
...
function gradientColorStops(tailwindColors) {
		...
		_.each(tailwindColors, (value, modifier) => {
			if (typeof value === 'string') {
				if (value === 'transparent') {
					convertedStyles += `'.${rule}${setModifier(modifier)}': { backgroundGradient: { colors: [ '${value}', '${value}' ] } }\n`;
				} else if (value !== 'currentColor') {
					convertedStyles += `'.${rule}${setModifier(modifier)}': { backgroundGradient: { colors: [ '${addTransparencyToHex(toHex(value))}', '${toHex(value)}' ] } }\n`;
				}
			} else {
				_.each(value, (shadeValue, shadeModifier) => {
					if (defaultOrDeviceModifier(shadeModifier)) {
						convertedStyles += `'.${rule}${setModifier(shadeModifier)}': { backgroundGradient: { colors: [ '${addTransparencyToHex(toHex(shadeValue))}', '${toHex(shadeValue)}' ] } }\n`;
					} else if (defaultOrDeviceModifier(modifier)) {
						convertedStyles += `'.${rule}${setModifier(shadeModifier)}${setModifier(modifier)}': { backgroundGradient: { colors: [ '${addTransparencyToHex(toHex(shadeValue))}', '${toHex(shadeValue)}' ] } }\n`;
					} else {
						convertedStyles += `'.${rule}${setModifier(modifier)}${setModifier(shadeModifier)}': { backgroundGradient: { colors: [ '${addTransparencyToHex(toHex(shadeValue))}', '${toHex(shadeValue)}' ] } }\n`;
					}
				});
			}
		});
	...
	_.each(rulesAndProperties, rule => {
		...
		_.each(tailwindColors, (value, modifier) => {
			if (typeof value === 'string' && value !== 'currentColor') {
				convertedStyles += `'.${rule}${setModifier(modifier)}': { backgroundGradient: { colors: [ '${toHex(value)}' ] } }\n`;
			} else if (value !== 'currentColor') {
				_.each(value, (shadeValue, shadeModifier) => {
					if (defaultModifier(shadeModifier)) {
						convertedStyles += `'.${rule}${setModifier(modifier)}': { backgroundGradient: { colors: [ '${toHex(shadeValue)}' ] } }\n`;
					} else if (defaultOrDeviceModifier(modifier)) {
						convertedStyles += `'.${rule}${setModifier(shadeModifier)}${setModifier(modifier)}': { backgroundGradient: { colors: [ '${toHex(shadeValue)}' ] } }\n`;
					} else {
						convertedStyles += `'.${rule}${setModifier(modifier)}${setModifier(shadeModifier)}': { backgroundGradient: { colors: [ '${toHex(shadeValue)}' ] } }\n`;
					}
				});
			}
		});
...
function fontSize(tailwindFontSizes) {
	...
	_.each(tailwindFontSizes, (value, modifier) => {
		if (Array.isArray(value)) {
			...
			_.each(value, (theValue, theModifier) => {
				if (defaultOrDeviceModifier(theModifier)) {
					convertedStyles += singleValue(`.text-${modifier}`, theModifier, 'font: { fontSize', `${parseValue(theValue)} }`);
				} else {
					convertedStyles += singleValue(`.text-${theModifier}`, modifier, 'font: { fontSize', `${parseValue(theValue)} }`);
				}
			});
		} else {
			if (typeof value === 'object') {
				_.each(value, (theValue, theModifier) => {
					convertedStyles += multiValues(`.text${setModifier(modifier)}`, theModifier, 'font', ` fontSize: '${theValue}'`);
				});
			} else {
...
function fontWeight(fontWeights) {
	...
	_.each(fontWeights, (value, modifier) => {
		if (defaultOrDeviceModifier(modifier)) {
			if (typeof value === 'object') {
				_.each(value, (theValue, theModifier) => {
					convertedStyles += singleValue(`.font-${theModifier}`, modifier, 'font: { fontWeight', `'${fixInvalidValues(invalidValues, theValue)}' }`);
				});
			} else {
	...
	_.each(spacing, (value, modifier) => {
		if (modifier !== 'min' && modifier !== 'max' && modifier !== 'min-content' && modifier !== 'max-content' && modifier !== 'auto' && modifier !== 'screen') {
			if (defaultOrDeviceModifier(modifier)) {
				if (typeof value === 'object') {
					_.each(value, (theValue, theModifier) => {
						convertedStyles += singleValue(`.rounded${setModifier(theModifier)}`, modifier, 'borderRadius', parseValue(theValue));
					});
				} else {
					convertedStyles += singleValue('.rounded', '', 'borderRadius', parseValue(value));
				}
			} else if (['sm', 'md', 'lg', 'xl', '2xl', '3xl'].includes(modifier)) {
...
function borderWidth(tailwindBorderWidth) {
	...
	_.each(tailwindBorderWidth, (value, modifier) => {
		if (defaultOrDeviceModifier(modifier)) {
			if (typeof value === 'object') {
				_.each(value, (theValue, theModifier) => {
					convertedStyles += singleValue(`.border-${theModifier}`, modifier, 'borderWidth', parseValue(theValue));
				});
			} else {
			...
			_.each(value, (theValue, theModifier) => {
				convertedStyles += singleValue(`.border-${modifier}`, theModifier, 'borderWidth', parseValue(theValue));
			});
...
function shadow() {
	...
	convertedStyles += `'.shadow-xs': { viewShadowOffset: { x: 0, y: 0 }, viewShadowRadius: 1, viewShadowColor: '#80000000' }\n`;
	convertedStyles += `'.shadow-sm': { viewShadowOffset: { x: 0, y: 1 }, viewShadowRadius: 1, viewShadowColor: '#80000000' }\n`;
	convertedStyles += `'.shadow': { viewShadowOffset: { x: 0, y: 2 }, viewShadowRadius: 2, viewShadowColor: '#80000000' }\n`;
	convertedStyles += `'.shadow-md': { viewShadowOffset: { x: 0, y: 4 }, viewShadowRadius: 4, viewShadowColor: '#80000000' }\n`;
	convertedStyles += `'.shadow-lg': { viewShadowOffset: { x: 0, y: 8 }, viewShadowRadius: 8, viewShadowColor: '#80000000' }\n`;
	convertedStyles += `'.shadow-xl': { viewShadowOffset: { x: 0, y: 12 }, viewShadowRadius: 12, viewShadowColor: '#80000000' }\n`;
	convertedStyles += `'.shadow-2xl': { viewShadowOffset: { x: 0, y: 16 }, viewShadowRadius: 16, viewShadowColor: '#80000000' }\n`;
	convertedStyles += `'.shadow-inner': { viewShadowOffset: { x: 0, y: 0 }, viewShadowRadius: null, viewShadowColor: null }\n`;
	convertedStyles += `'.shadow-outline': { viewShadowOffset: { x: 0, y: 0 }, viewShadowRadius: 2, viewShadowColor: '#80000000' }\n`;
...
function opacity(tailwindOpacity) {
	...
	_.each(tailwindOpacity, (value, modifier) => {
		if (typeof value === 'object') {
			_.each(value, (theValue, theModifier) => {
				convertedStyles += singleValue(`.opacity-${theModifier}`, modifier, 'opacity', parseValue(theValue));
			});
		} else {
				...
				if (typeof value === 'object') {
					_.each(value, (theValue, theModifier) => {
						...
						_.each(properties, property => {
							sides += (modifier === 'px')
								? ` ${property}: '1px',`
								: ` ${property}: ${parseValue(theValue)},`;
						});

						convertedStyles += multiProperties(`.${rule}${setModifier(theModifier)}`, modifier, remove_last_character(sides));
					});
				} else {
					let sides = '';
					...
					_.each(value, (theValue, theModifier) => {
						...
						_.each(properties, property => {
							if (parseFloat(theValue) !== 0) {
								sides += (modifier === 'px')
									? ` ${property}: '-1px',`
									: (!modifier.includes('/') && modifier !== 'full') ? ` ${property}: ${parseValue(theValue, '-')},` : ` ${property}: ${parseValue(theValue, '-')},`;
							}
						});
			...
			if (modifier !== 'min' && modifier !== 'max' && modifier !== 'min-content' && modifier !== 'max-content' && modifier !== 'screen' && modifier !== 'auto') {
				if (typeof value === 'object') {
					_.each(value, (theValue, theModifier) => {
						...
						convertedStyles += multiValues(`.${rule}${setModifier(theModifier)}`, modifier, 'padding', remove_last_character(sides));
					});
				} else {
					let sides = '';
	...
	_.each(_value, (value, modifier) => {
		...
		_.each(value, (theValue, theModifier) => {
			if (typeof (theValue) === 'object' && theValue !== null) {
				...
				_.each(theValue, (extraValue, modifier) => {
...
function defaultOrDeviceModifier(modifier) {
	return defaultModifier(modifier) || deviceModifier(modifier);
}
...
function deviceModifier(modifier) {
	return modifier === 'ios' || modifier === 'android' || modifier.includes('[');
}
	...
	_.each(data, (value, modifier) => {
		if (modifier !== 'min' && modifier !== 'max' && modifier !== 'min-content' && modifier !== 'max-content') {
			if (modifier === 'screen') {
				...
				if (typeof value === 'object') {
					_.each(value, (theValue, theModifier) => {
						convertedStyles += singleValue(`.${rule}${theModifier}`, modifier, property, parseValue(theValue));
					});
				} else {
...
function singleValue(rule, modifier, property, value) {
	return `'${rule}${setModifier(modifier)}': { ${property}: ${value} }\n`;
}

function multiValues(rule, modifier, property, values) {
	return `'${rule}${setModifier(modifier)}': { ${property}: {${values} } }\n`;
}

function multiProperties(rule, modifier, values) {
	return `'${rule}${setModifier(modifier)}': {${values} }\n`;
}
