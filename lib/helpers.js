...
function borderWidth(tailwindBorderWidth) {
	...
	_.each(tailwindBorderWidth, (value, modifier) => {
		convertedStyles += (modifier === 'default')
			? singleValue('.border', '', 'borderWidth', "'1px'")
...
function customRules(_value, _key) {
	...
	_.each(_value, (value, modifier) => {
		...
		if (modifier === 'global' || modifier === 'default') {
			modifier = '';
...
function checkTitanium(value) {
	let substrings = ['Alloy', 'Ti'];

	if (substrings.some(function (v) { return value.indexOf(v) >= 0; })) {
		return 'titanium';
...
function parseValue(value, sign = '') {
	...
	let unit = isNaN(value) ? checkTitanium(value.replace(/[^a-zA-Z#%]+/g, '')) : value;
	...
	switch (unit) {
		...
		default:
			// Last time check for a number or a string
			return isNaN(value) ? `'${sign}${value}'` : `${sign}${value}`;
	}
}

function processElements(header, rule, property, data) {
	let allTogether = { ...data, ...data.spacing };
	delete allTogether.spacing;
	let convertedStyles = `\n// ${header}\n`;

	_.each(allTogether, (value, modifier) => {
		if (modifier === 'screen') {
