	...
	_.each(values, (value, modifier) => {
		if (modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT' || modifier === 'ios' || modifier === 'android' || modifier.includes('[')) {
			if (typeof value === 'object') {
				_.each(value, (theValue, theProperty) => {
					convertedStyles += multiValues(`.font-${theProperty}`, checkModifier(modifier), 'font', ` fontFamily: '${theValue}'`);
				});
			} else {
			...
			if (typeof value === 'object') {
				_.each(value, (theValue, theProperty) => {
					convertedStyles += multiValues(`.font-${modifier}`, `-${checkModifier(theProperty)}`, 'font', ` fontFamily: '${theValue}'`);
				});
			} else {
				convertedStyles += multiValues('.font-', modifier, 'font', ` fontFamily: '${value}'`);
			}
...
function fontSize(tailwindFontSizes) {
	...
	_.each(tailwindFontSizes, (value, modifier) => {
		if (Array.isArray(value)) {
			...
			_.each(value, (theValue, theProperty) => {
				if (theProperty === 'global' || theProperty === 'default' || theProperty === 'DEFAULT' || theProperty === 'ios' || theProperty === 'android' || theProperty.includes('[')) {
					// convertedStyles += `'.${rule}${modifier}${checkModifier(theProperty)}': { backgroundGradient: { colors: [ '${addTransparencyToHex(toHex(shadeValue))}', '${toHex(shadeValue)}' ] } }\n`;
					convertedStyles += singleValue(`.text-${modifier}`, checkModifier(theProperty), 'font: { fontSize', `${parseValue(theValue)} }`);
				} else {
					convertedStyles += singleValue(`.text-${theProperty}`, checkModifier(modifier), 'font: { fontSize', `${parseValue(theValue)} }`);
				}
			});
		} else {
			if (typeof value === 'object') {
				_.each(value, (theValue, theProperty) => {
					convertedStyles += multiValues(`.text-${modifier}`, checkModifier(theProperty), 'font', ` fontSize: '${theValue}'`);
				});
			} else {
...
function fontWeight(fontWeights) {
	...
	_.each(fontWeights, (value, modifier) => {
		if (modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT' || modifier === 'ios' || modifier === 'android' || modifier.includes('[')) {
			if (typeof value === 'object') {
				_.each(value, (theValue, theProperty) => {
					convertedStyles += singleValue(`.font-${theProperty}`, checkModifier(modifier), 'font: { fontWeight', `'${fixInvalidValues(invalidValues, theValue)}' }`);
				});
			} else {
	...
	_.each(spacing, (value, modifier) => {
		if (modifier !== 'min' && modifier !== 'max' && modifier !== 'min-content' && modifier !== 'max-content' && modifier !== 'auto' && modifier !== 'screen') {
			if (modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT' || modifier === 'ios' || modifier === 'android' || modifier.includes('[')) {
				if (typeof value === 'object') {
					_.each(value, (theValue, theProperty) => {
						convertedStyles += singleValue(`.rounded-${theProperty}`, checkModifier(modifier), 'borderRadius', parseValue(theValue));
					});
				} else {
					convertedStyles += singleValue('.rounded', '', 'borderRadius', parseValue(value));
				}
...
function borderWidth(tailwindBorderWidth) {
	...
	_.each(tailwindBorderWidth, (value, modifier) => {
		if (modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT' || modifier === 'ios' || modifier === 'android' || modifier.includes('[')) {
			if (typeof value === 'object') {
				_.each(value, (theValue, theProperty) => {
					convertedStyles += singleValue(`.border-${theProperty}`, checkModifier(modifier), 'borderWidth', parseValue(theValue));
				});
			} else {
			...
			_.each(value, (theValue, theProperty) => {
				convertedStyles += singleValue(`.border-${modifier}`, checkModifier(theProperty), 'borderWidth', parseValue(theValue));
			});
...
function opacity(tailwindOpacity) {
	...
	_.each(tailwindOpacity, (value, modifier) => {
		if (typeof value === 'object') {
			_.each(value, (theValue, theProperty) => {
				convertedStyles += singleValue(`.opacity-${theProperty}`, checkModifier(modifier), 'opacity', parseValue(theValue));
			});
		} else {
			...
			if (modifier !== 'min' && modifier !== 'min-content' && modifier !== 'max' && modifier !== 'max-content' && modifier !== 'screen' && modifier !== 'auto') {
				if (typeof value === 'object') {
					_.each(value, (theValue, theProperty) => {
						...
						convertedStyles += multiProperties(`.${rule}-${theProperty}`, modifier, remove_last_character(sides));
					});
				} else {
					let sides = '';
			...
			if (modifier !== 'min' && modifier !== 'max' && modifier !== 'min-content' && modifier !== 'max-content' && modifier !== 'screen' && modifier !== 'auto') {
				if (typeof value === 'object') {
					_.each(value, (theValue, theProperty) => {
						...
						convertedStyles += multiValues(`.${rule}-${theProperty}`, modifier, 'padding', remove_last_character(sides));
					});
				} else {
					let sides = '';
	...
	_.each(_value, (value, modifier) => {
		...
		_.each(value, (theValue, theProperty) => {
			if (typeof (theValue) === 'object' && theValue !== null) {
				...
				customProperties += ` ${theProperty}: ${parseValue(theValue)},`;
			}
		});
	...
	_.each(data, (value, modifier) => {
		if (modifier !== 'min' && modifier !== 'max' && modifier !== 'min-content' && modifier !== 'max-content') {
			if (modifier === 'screen') {
				...
				if (typeof value === 'object') {
					_.each(value, (theValue, theProperty) => {
						convertedStyles += singleValue(`.${rule}${theProperty}`, checkModifier(modifier), property, parseValue(theValue));
					});
				} else {
