		...
		_.each(tailwindColors, (value, modifier) => {
			if (typeof value === 'string') {
				if (value === 'transparent') {
					...
					convertedStyles += `'.${rule}${modifier}': { backgroundGradient: { colors: [ '` + toTransparencyZero(value) + `', '${value}' ] } }\n`;
				}
			} else {
				_.each(value, (shadeValue, shadeName) => {
					if (shadeName === 'global' || shadeName === 'default' || shadeName === 'DEFAULT') {
						convertedStyles += `'.${rule}${modifier}': { backgroundGradient: { colors: [ '` + toTransparencyZero(shadeValue) + `', ${parseValue(shadeValue)} ] } }\n`;
					} else {
						if (modifier === 'ios' || modifier === 'android' || modifier.includes('[') || modifier === 'global' || modifier === 'default' || modifier === 'DEFAULT') {
							convertedStyles += `'.${rule}${shadeName}${checkModifier(modifier)}': { backgroundGradient: { colors: [ '` + toTransparencyZero(shadeValue) + `', ${parseValue(shadeValue)} ] } }\n`;
						} else {
							convertedStyles += `'.${rule}${modifier}-${shadeName}': { backgroundGradient: { colors: [ '` + toTransparencyZero(shadeValue) + `', ${parseValue(shadeValue)} ] } }\n`;
						}
					}
				});
			}
...
function toTransparencyZero(color) {
	try {
		switch (color.length) {
			case 4:
		...
		if (color.match(/rgba?/i)) {
			color = toTransparencyZero(rgbToHex(color));
		}

		return color;
	} catch (error) {
		console.log('Error:', color, position);
	}
}
