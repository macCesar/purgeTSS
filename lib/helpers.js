			...
			_.each(rulesAndValuesPair, (value, rule) => {
				...
				if (typeof value === 'object') {
					_.each(value, (_value, key) => {
						...
						convertedStyles += defaultModifier(key) ? `'.${setModifier2(mainSelector, rule)}${setModifier2(rule)}${setModifier2(selector, rule)}': ${valores}\n` : `'.${setModifier2(mainSelector, rule)}${setModifier2(rule)}-${key}${setModifier2(selector, rule)}': ${valores}\n`;
					});
				} else {
					let valores = _.replace(declarationBlock, new RegExp("{value}", "g"), parseValue(value, minusSigns));
					convertedStyles += `'.${setModifier2(mainSelector, rule)}${setModifier2(rule)}${setModifier2(selector)}': ${valores}\n`;
				}
			});
		});
	});
...
function setModifier2(modifier, rule = null) {
	if (defaultModifier(modifier)) {
		...
		modifier = `${modifier}`;
	} else if (!modifier.endsWith('-') && rule !== null && rule !== 'default') {
		modifier = `${modifier}-`;
	}
