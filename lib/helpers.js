...
function compileApplyDirectives(twStyles) {
	...
	let fontsStylesArray = (fs.existsSync(cwd + '/purgetss/fonts.tss')) ? fs.readFileSync(cwd + '/purgetss/fonts.tss', 'utf8').split(/\r?\n/) : null;
				...
				if (searchClass.includes('(')) {
					...
					if (indexOfClass !== -1) {
						compoundClasses.push(twStylesArray[indexOfClass]
							...
							.replace(/{(.*)}/, '$1')
							.trim());
					} else if (fontsStylesArray) {
						indexOfClass = findIndexOf(`'.${searchClass}': `, fontsStylesArray);
						if (indexOfClass !== -1) {
							compoundClasses.push(fontsStylesArray[indexOfClass]
								.replace(`'.${searchClass}': `, '')
								.replace(/{(.*)}/, '$1')
								.trim());
						}
