...
(function constructor() {
	...
	readCSS('./node_modules/boxicons/css/boxicons.css', (err, data) => {
		if (err) throw err

		let tssClasses = fs.readFileSync('./lib/templates/boxicons/template.tss', 'utf8') + '\n';

		tssClasses += fs.readFileSync('./lib/templates/boxicons/reset.tss', 'utf8');

		tssClasses += processCSS(data);

		fs.writeFileSync('./dist/boxicons.tss', tssClasses, err => {
			throw err;
		});

		console.log(`${purgeLabel} './dist/boxicons.tss' file created!`);
	});
}());
